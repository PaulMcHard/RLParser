...
for epoch in range(total_epochs):
    ...
    temp_err = np.zeros((2, state_size))
    for set in range(len(train_sets)-1):
        ...
        for step in range(state_size-1):
            ...
            temp_xfbk[step] = train_sets[set].xfbk[step] + actions[x_action]*np.sign(train_sets[set].xcom[step]-train_sets[set].xfbk[step]) # add 1, 0 or -1 to demonstrate accel, const or decel
            temp_yfbk[step] = train_sets[set].yfbk[step] + actions[y_action]*np.sign(train_sets[set].ycom[step]-train_sets[set].yfbk[step])
            temp_err[0, step] = np.absolute(train_sets[set].xcom[step]-temp_xfbk[step])
            temp_err[1, step] = np.absolute(train_sets[set].ycom[step]-temp_yfbk[step])
            temp_err[0, step+1] = np.absolute(train_sets[set].xcom[step+1]-train_sets[set].xfbk[step+1])
            temp_err[1, step+1] = np.absolute(train_sets[set].ycom[step+1]-train_sets[set].yfbk[step+1])
            x_reward = check_error(temp_err[0,step], temp_err[0,step+1])
            y_reward = check_error(temp_err[1,step], temp_err[1,step+1])
...
plt.scatter(X, x_end_mean, color='k')
plt.scatter(X, y_end_mean, color='g')
